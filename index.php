...
echo $query;	
	...
	if ( isset($tot_calls_raw) && $tot_calls_raw ) {
		...
		try {
			...
			$query = "SELECT *, datetime(calldate) as call_timestamp FROM $db_table_name $where $order $sort LIMIT $result_limit";
			$sth = $dbh->query($query);
			if (!$sth) {
				echo "\nPDO::errorInfo():\n";
				print_r($dbh->errorInfo());
...
switch ($group) {
	case "disposition_by_day":
		...
		$group_by_field = "CONCAT(datetime(calldate, '$group_by_field_php[0]'),disposition)";
	break;
	...
	case "hour":
		...
		$group_by_field = "datetime(calldate, '$group_by_field_php[0]')";
		$graph_col_title = 'Час';
	break;
	case "hour_of_day":
	...
	case "day_of_week":
		...
		$group_by_field = "datetime( calldate, '%w - %W' )";
		$graph_col_title = 'День недели';
	break;
	case "minutes1":
		$group_by_field_php = array( '%Y-%m-%d %H:%M', 16, '' );
		$group_by_field = "datetime(calldate, '%Y-%m-%d %H:%i')";
		$graph_col_title = 'Минута';
	break;
	case "minutes10":
		$group_by_field_php = array('%Y-%m-%d %H:%M',15,'0');
		$group_by_field = "CONCAT(SUBSTR(datetime(calldate, '%Y-%m-%d %H:%i'),1,15), '0')";
